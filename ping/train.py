...
if __name__ == '__main__':
	...
	os.makedirs("./model/2"+target_domain, exist_ok=True)
	...
	for ep in range(EP):
		...
		torch.save(extractor.state_dict(), './model/2'+target_domain+'/extractor'+'_'+str(ep)+'.pth')
		for source in source_domain:
			torch.save(source_clf[source]['c1'].state_dict(), './model/2'+target_domain+'/'+source+'_c1_'+str(ep)+'.pth')
			torch.save(source_clf[source]['c2'].state_dict(), './model/2'+target_domain+'/'+source+'_c2_'+str(ep)+'.pth')
	...
	for source in source_domain:
		for i in range(1, 3):
			...
			plt.xlabel('EP')
		plt.legend()
		plt.savefig('./model/2' + target_domain + '/' + source + '_loss_ac.jpg')
		plt.show()
	...
	plt.xlabel('EP')
	plt.ylabel('loss')
	plt.savefig('./model/2' + target_domain + '/discrepency_loss.jpg')
	plt.show()
	...
	plt.xlabel('EP')
	plt.ylabel('loss')
	plt.savefig('./model/2' + target_domain + '/mc_loss.jpg')
	plt.show()
